...
data "terraform_remote_state" "aws_api_gateway_parent_api" {
  backend = var.src.backend
  config  = try(local.config_parent_api, {})
}
...
data "terraform_remote_state" "aws_lb_target_group_mgr_message_category" {
  backend = var.src.backend
