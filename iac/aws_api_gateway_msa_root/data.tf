...
data "terraform_remote_state" "aws_lb_target_group_msa_ping" {
  backend = var.src.backend
  config  = try(local.config_tg_ping, {})
}

data "terraform_remote_state" "aws_lb_target_group" {
  backend = var.src.backend
  config  = try(local.config_tg_msg, {})
}
