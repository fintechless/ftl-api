locals {
  ...
  parent_id     = data.terraform_remote_state.aws_api_gateway_rest_api.outputs.root_resource_id
  ...
  msa_port = {
    get  = data.terraform_remote_state.aws_lb_target_group_msa_ping.outputs.port
    post = data.terraform_remote_state.aws_lb_target_group.outputs.port
  }
  ...
  config_tg_ping = {
    region = data.aws_region.this.name
    bucket = local.ftl_bucket
    key    = var.src.config_key_tg_ping
  }

  config_tg_msg = {
    region = data.aws_region.this.name
    bucket = local.ftl_bucket
    key    = var.src.config_key_tg_msg
  }
}
