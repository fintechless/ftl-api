locals {
  ...
  deployment_labels = data.terraform_remote_state.aws_k8s_deployment_msa_mapping.outputs.labels
  ...
  ingress_name      = local.dockercontainers.msa[var.src.msa].igr_name
  ...
  health_check_path = local.dockercontainers.msa[var.src.msa].health_check_path
